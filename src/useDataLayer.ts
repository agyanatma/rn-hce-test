...
const useDataLayer = (): DataLayer => {
    ...
    useEffect(() => {
        console.log('nfcTagProps._pristine');
    ...
    useEffect(() => {
        console.log(HCESession.Events.HCE_STATE_WRITE_FULL);
    ...
    const logger: HCESessionEventListener = useCallback(
        (eventData) => {
            if (!eventData) return;
